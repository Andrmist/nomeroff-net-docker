...
def read_number_plates(urls):
    ...
    for url_id in range(len(urls)):
        res = {
            "url": urls[url_id],
            # "images": images[url_id].tolist(),
            "images_bboxs": images_bboxs[url_id].tolist(),
            # "images_zones": (images_zones[url_id]),
            "region_ids": (region_ids[url_id]),
            "region_names": (region_names[url_id]),
            "count_lines": (count_lines[url_id]),
            "confidences": (confidences[url_id]),
            "texts": (texts[url_id]),
        }
        print(res)
        numberplates.append(res)
    return numberplates
    ...
    #     areas = [
    #         abs(x2 - x1) * abs(y2 - y1)
    #         for x1, y1, x2, y2, confidence, class_id in images_bbox
    #     ]
